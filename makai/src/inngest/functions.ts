...
// In the inngest background function this will fail because we dont have any active credits from openai, so the summary is not generated.
...
export const meetingsProcessing = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const { output } = await summerizer.run(
      'Summerize the following meeting transcript' +
